...
if __name__ == "__main__": 
	...
	for index in range(0,1	):
		...
		acc_dic["Decision Tree"].append(acc_dt)
		acc_dic["Logistic"].append(acc_logreg)
		acc_dic["LogisticCV"].append(acc_logreg_cv)
		acc_dic["Bayes"].append(acc_bayes)

	output_file = open("accuracies.csv", "w")
	output_file.write("classifier,mean,std,accuracies\n")
	output_file.write("DecisionTree,"+str(np.mean(acc_dic["Decision Tree"]))+","+str(np.std(acc_dic["Decision Tree"]))+",".join(str(x) for x in acc_dic["Decision Tree"])+"\n")
	#output_file.write("ExtraTree,"+str(acc_et)+"\n")
	#output_file.write("RandomForest,"+str(acc_rf)+"\n")
	output_file.write("Logistic,"+str(np.mean(acc_dic["Logistic"]))+","+str(np.std(acc_dic["Logistic"]))+",".join(str(x) for x in acc_dic["Logistic"])+"\n")
	output_file.write("Logistic,"+str(np.mean(acc_dic["LogisticCV"]))+","+str(np.std(acc_dic["LogisticCV"]))+",".join(str(x) for x in acc_dic["LogisticCV"])+"\n")
	output_file.write("Bayes,"+str(np.mean(acc_dic["Bayes"]))+","+str(np.std(acc_dic["Bayes"]))+",".join(str(x) for x in acc_dic["Bayes"])+"\n")
	output_file.close()
