...
if __name__ == "__main__": 
	...
	for index in range(0,30):
		...
		acc_dic["Extra Trees"].append(acc_et)

	output_file = open("accuracies-et.csv", "w")
	output_file.write("classifier,mean,std,accuracies\n")
	output_file.write("ExtraTrees,"+str(np.mean(acc_dic["Extra Trees"]))+","+str(np.std(acc_dic["Extra Trees"]))+",".join(str(x) for x in acc_dic["Extra Trees"])+"\n")
	output_file.close()
